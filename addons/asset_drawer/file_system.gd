...
var AssetDrawerShortcut: InputEventMouseButton = InputEventMouseButton.new()
var HidePanelShortcut: InputEventMouseButton = InputEventMouseButton.new()
...
func _enter_tree() -> void:
	...
	# Get shortcut
	AssetDrawerShortcut = preload("res://addons/asset_drawer/asset_drawer_shortcut.tres")
	HidePanelShortcut = preload("res://addons/asset_drawer/hide_panel_shortcut.tres")
		...
		if filesBottom == true:
			match showing:
				...
				true:
					...
					hide_bottom_panel()
					showing = false
	if HidePanelShortcut.is_match(event) and event.is_released() and not event.is_echo():
		hide_bottom_panel()
		showing = false
#endregion
